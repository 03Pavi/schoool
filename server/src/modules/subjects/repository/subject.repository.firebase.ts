...
const memoryDb = new Map<string, Subject>();

export class SubjectFirebaseRepository implements ISubjectRepository {
  ...
  async findAll(options: QueryOptions = {}): Promise<PaginatedResult<Subject>> {
    ...
    if (db) {
      ...
      items = snapshot.docs.map(doc => doc.data() as Subject);
  ...
  async update(id: string, data: Partial<Subject>): Promise<Subject> {
    if (db) {
      ...
      if (!existing.exists || existing.data()?.isArchived) throw new Error('Subject not found');
    ...
    if (!existing || existing.isArchived) throw new Error('Subject not found');
